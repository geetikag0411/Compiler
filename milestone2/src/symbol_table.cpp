...
st_entry::st_entry(string name, Type type, int offset, int line_no, symbol_table *table, bool is_initialized) : name(name), type(type), offset(offset), line_no(line_no), table(table), is_initialized(is_initialized) {
    if(type.is_list||type.datatype=="str")
    {
        size = 8;
    }
...
void symbol_table_global::make_csv()
{
    ...
    file << "Syntactic Category(token),Lexeme,Type,Line No,Offset\n";
    for (auto &[name, st_entry] : entries)
    {
    ...
    {
        file<<"Class,"<<clas.first<<",Not Defined,"<<clas.second->get_line_no()<<",None\n";
